...
class UserInteractionRepositoryImpl @Inject constructor(
    ...
    override suspend fun getFavoriteStatus(oreumIdx: String): Boolean {
        ...
        val map = doc.get(FirestoreConstants.FIELD_FAVORITES).toStringBooleanMap()
    ...
    override suspend fun getStampStatus(oreumIdx: String): Boolean {
        ...
        val map = doc.get(FirestoreConstants.FIELD_STAMPED_OREUMS).toStringStringMap()
    ...
    override suspend fun toggleFavorite(oreumIdx: String, newIsFavorite: Boolean): Int {
        ...
        return try {
            firestore.runTransaction { tx ->
                ...
                val favorites =
                    userSnap.get(FirestoreConstants.FIELD_FAVORITES).toStringBooleanMap().toMutableMap()
                var count = oreumSnap.getLong(FirestoreConstants.FIELD_FAVORITE)?.toInt() ?: 0
                ...
                tx.set(
                    userDoc,
                    mapOf(FirestoreConstants.FIELD_FAVORITES to favorites), SetOptions.merge()
                )
                tx.set(
                    oreumDoc,
                    mapOf(FirestoreConstants.FIELD_FAVORITE to count), SetOptions.merge()
                )
    ...
    override suspend fun getAllFavoriteStatus(): Map<String, Boolean> {
        ...
        return doc.get(FirestoreConstants.FIELD_FAVORITES).toStringBooleanMap()
    }
    ...
    override suspend fun getAllStampStatus(): Map<String, Boolean> {
        ...
        return doc.get(FirestoreConstants.FIELD_STAMPED_OREUMS).toStringStringMap().mapValues { true }
    }
