...
class UserInteractionRepositoryImpl(
    ...
    override suspend fun getFavoriteStatus(oreumIdx: String): Boolean {
        ...
        val doc = firestore.collection("user_info_col").document(uid).get().await()
    ...
    override suspend fun toggleFavorite(oreumIdx: String, newIsFavorite: Boolean): Int {
        ...
        val userDoc = firestore.collection("user_info_col").document(uid)
        val oreumDoc = firestore.collection("oreum_info_col").document(oreumIdx)

        return firestore.runTransaction { tx ->
            val userSnap = tx.get(userDoc)
            val oreumSnap = tx.get(oreumDoc)

            val favorites = (userSnap.get("favorites") as? Map<String, Boolean>)?.toMutableMap() ?: mutableMapOf()
            ...
            tx.set(userDoc, mapOf("favorites" to favorites), SetOptions.merge())
            tx.set(oreumDoc, mapOf("favorite" to count), SetOptions.merge())
            count
        }.await()
    }
